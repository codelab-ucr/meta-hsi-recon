...
import cupy
from cupyx.scipy import signal as signal_cu
...
def convsep_gpu(im, win_h, win_w):
    '''
        Implement separable 2D convolution for guided filtering on GPU

        Inputs:
            im: Image to filter
            win_h: Horizontal filter
            win_w: Wertical filter XD
    '''
    im_h = signal_cu.convolve2d(im, win_h.reshape(-1, 1), mode='same')
    im_w = signal_cu.convolve2d(im_h, win_w.reshape(1, -1), mode='same')
...
def guidedFilterMasked_gpu(guide, img, mask, winsize=7, eps=1e-6,
                           wintype='gaussian'):
    '''
        Reconstruct an image using masked guided filtering approach
        ...
        Outputs:
            filtered_img: Filtered image
    '''
    # Precompute convolutional kernels
    if wintype == 'gaussian':
        win_h = signal.gaussian(winsize, winsize/3)
        win_w = signal.gaussian(winsize, winsize/3)
    else:
        win_h = np.ones(winsize)
        win_w = np.ones(winsize)
        
    # Send all to GPU
    win_h = cupy.array(win_h)
    win_w = cupy.array(win_w)
    
    guide = cupy.array(guide)
    img = cupy.array(img)
    mask = cupy.array(mask)
        
    # Compute individual terms
    im11 = convsep_gpu(mask, win_h, win_w)

    imxy = convsep_gpu(guide*img*mask, win_h, win_w)/im11
    imxx = convsep_gpu(guide*guide*mask, win_h, win_w)/im11

    imx1 = convsep_gpu(guide*mask, win_h, win_w)/im11
    imy1 = convsep_gpu(img*mask, win_h, win_w)/im11

    im_alpha = (imxy - imx1*imy1)/(imxx - imx1*imx1 + eps)
    im_beta = imy1 - im_alpha*imx1
    
    return (im_alpha*guide + im_beta).get()
